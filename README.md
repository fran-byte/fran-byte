![hi](profile2.png)

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://linkedin.com/in/francisco-jesus-romero-caceres)
[![Email](https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:francisco.jesus.romero.caceres@hotmail.com)
[![42 Madrid](https://img.shields.io/badge/42_Madrid-000000?style=for-the-badge&logo=42&logoColor=white)](https://42madrid.com/)

---

# 🔧 Senior Electronics Specialist → Software Developer

> **25+ years** diagnosing, repairing and maintaining complex electronic systems.  
> Currently training in **software development** at **42 Madrid**.  
> Bridging the gap between **hardware, embedded systems and software** to build innovative solutions.

---

## 🎯 About Me

With over two decades of experience as a **Senior Electronics Technician**, I've developed a deep understanding of:

- 🔬 **PCB diagnostics** at component level
- ⚡ **Power supplies** and analog/digital circuits
- 🏦 **Complex electromechanical systems** including ATMs and cash recyclers
- 🔧 **Electronic troubleshooting and fault diagnosis**
- 📡 **Embedded systems and industrial communication protocols**
- 🧪 **Test, measurement and equipment calibration**
- 🔌 **Serial and board-level communication interfaces**

Today, I'm combining this hardware expertise with **software development skills** acquired through the rigorous peer-to-peer methodology at **42 Madrid**.

My goal is to bridge the gap between **electronics and software**, creating reliable systems where hardware, firmware and applications work together.

---

## 🛠️ Tech Stack

### 💻 Programming Languages

![C](https://skillicons.dev/icons?i=c)
![C++](https://skillicons.dev/icons?i=cpp)
![Python](https://skillicons.dev/icons?i=python)
![Bash](https://skillicons.dev/icons?i=bash)

---

### 🐧 Tools & Frameworks

![Linux](https://skillicons.dev/icons?i=linux)
![Git](https://skillicons.dev/icons?i=git)
![GitHub](https://skillicons.dev/icons?i=github)
![Docker](https://skillicons.dev/icons?i=docker)
![CMake](https://skillicons.dev/icons?i=cmake)
![VS Code](https://skillicons.dev/icons?i=vscode)
![Nginx](https://skillicons.dev/icons?i=nginx)
![Cloudflare](https://skillicons.dev/icons?i=cloudflare)

---

### 🔌 Hardware & Embedded

![Arduino](https://skillicons.dev/icons?i=arduino)
![Raspberry Pi](https://skillicons.dev/icons?i=raspberrypi)

---

### 📡 Protocols & Interfaces

![I2C](https://img.shields.io/badge/I²C-Protocol-555555?style=for-the-badge)
![UART](https://img.shields.io/badge/UART-Serial-555555?style=for-the-badge)
![SPI](https://img.shields.io/badge/SPI-Protocol-555555?style=for-the-badge)
![RS232](https://img.shields.io/badge/RS--232-Serial-555555?style=for-the-badge)
![TCP/IP](https://img.shields.io/badge/TCP%2FIP-Networking-555555?style=for-the-badge)
![WiFi](https://img.shields.io/badge/Wi--Fi-IoT-555555?style=for-the-badge)

---

## 🚀 Featured Projects

### 🏃 [Reaction Time System](https://github.com/fran-byte/reaction-time)

> **Measurement of Reaction Time in Starting Blocks**

An embedded system designed to measure reaction time in athletics starting blocks.

- ⚙️ ESP8266-based embedded device
- 📐 ADXL345 accelerometer
- 📡 I²C sensor communication
- 📶 Wi-Fi connectivity
- 🌐 Web interface for real-time visualization
- 🏃 Designed for athletics training and performance analysis

**Tech:** `C++` `Arduino` `ESP8266` `I²C` `ADXL345` `Wi-Fi` `Web Server`

---

### 🐚 [minishell](https://github.com/fran-byte/42-minishell)

> **Custom Unix Shell Implementation**

A Unix shell developed from scratch as part of the 42 Common Core.

- 🔀 Pipes
- 📥 Input/output redirections
- 🌱 Environment variable management
- 📜 Command history
- ⚡ Signal handling
- 🔧 Unix system calls

**Tech:** `C` `POSIX` `Linux` `System Calls`

---

### 🎮 [cub3D](https://github.com/fran-byte/42-cub3D)

> **3D Graphics Engine with Raycasting**

A Wolfenstein-style 3D rendering engine developed using raycasting techniques.

- 🧭 Raycasting engine
- 🧱 Texture mapping
- 👾 Sprite management
- 🎮 Keyboard and mouse interaction
- 🖥️ Real-time rendering
- 📐 Mathematical geometry and vector calculations

**Tech:** `C` `Raycasting` `Graphics Programming` `MiniLibX`

---

### 🧲 [MagSenseUI](https://github.com/fran-byte/MagSenseUI)

> **Diagnostic Interface for Magnetic Sensors**

A graphical diagnostic and calibration interface for magnetic sensors.

- 📊 Real-time sensor visualization
- 🧪 Sensor calibration
- 🔌 Serial communication
- 🖥️ Desktop graphical interface
- 📈 Diagnostic tools

**Tech:** `Python` `PyQt` `Serial Communication`

---

### 📚 [libft](https://github.com/fran-byte/42-libft)

> **Custom C Standard Library**

A custom implementation of many standard C library functions developed as part of the 42 Common Core.

- 🧵 String manipulation
- 🧠 Memory management
- 🔗 Linked lists
- 🛠️ Utility functions
- 🧪 Testing and validation

**Tech:** `C` `Make` `Memory Management` `Data Structures`

---

## 📊 GitHub Stats

<div align="center">

<img src="https://github-readme-streak-stats.herokuapp.com/?user=fran-byte&theme=tokyonight&hide_border=true" alt="GitHub Streak" width="48%">

</div>

---

## 🏆 42 Madrid — Common Core

| Project | Description | Status |
|---------|-------------|--------|
| [transcender](https://github.com/fran-byte/42-ft-Transcender) | Real-time multiplayer blackjack platform | ✅ Complete |
| [libft](https://github.com/fran-byte/42-libft) | Custom C library | ✅ Complete |
| [minishell](https://github.com/fran-byte/42-minishell) | Unix shell implementation | ✅ Complete |
| [cub3D](https://github.com/fran-byte/42-cub3D) | 3D raycasting engine | ✅ Complete |
| [push_swap](https://github.com/fran-byte/42-push_swap) | Sorting algorithm | ✅ Complete |
| [so_long](https://github.com/fran-byte/42_so_long) | 2D game with MiniLibX | ✅ Complete |
| [pipex](https://github.com/fran-byte/42-pipex) | Unix pipelines | ✅ Complete |

---

## 🔬 Electronics & Embedded Expertise

My background in electronics gives me a different perspective when developing software and embedded systems.

### Hardware

- 🔬 Component-level PCB troubleshooting
- ⚡ Analog and digital electronics
- 🔋 Power supply diagnosis
- 🔧 Soldering and rework
- 📐 Electronic measurements
- 🧪 Test and diagnostic equipment
- 🏭 Industrial and electromechanical systems

### Embedded

- 🧠 Microcontrollers
- 🔌 Sensor interfaces
- 📡 Serial communication
- 📶 Wireless communication
- 🌐 Embedded web interfaces
- 🛠️ Hardware/software integration

### Communication

`I²C` · `UART` · `SPI` · `CAN` · `RS-232` · `RS-485` · `Modbus` · `TCP/IP`

---

## 📈 Current Focus

```text
Electronics
    │
    ├── Hardware Diagnostics
    ├── Analog & Digital Electronics
    ├── Embedded Systems
    │
    ▼
Firmware
    │
    ├── C / C++
    ├── Microcontrollers
    ├── Sensors
    └── Communication Protocols
    │
    ▼
Software
    │
    ├── Linux
    ├── Systems Programming
    ├── Python
    ├── Networking
    └── DevOps
